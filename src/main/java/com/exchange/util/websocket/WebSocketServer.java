...
public class WebSocketServer {
	...
	@Autowired
	public void setVisitorService(VisitorService visitorService){ WebSocketServer.visitorService=visitorService;}
	...
	public void onOpen(Session session, @PathParam("username") String username) {
		...
		visitor.setTotalNum(sessionMap.size());
		visitor.setVisitTime(LocalDateTime.now());
		visitor.setVisitType(0);
		visitorService.addOne(visitor);
		...
		for (Object key : sessionMap.keySet()) {
			...
			System.out.println(key);
			Accounts account=accountService.getOne(new QueryWrapper<Accounts>().eq("name",key));
//			System.out.println("avatar");
//			System.out.println(avatar);
			jsonObject.set("avatar",account.getAvatar());
			jsonObject.set("accountType",account.getAccountType().getDisplay());
...
//        {"users": [{"username": "zhang"},{ "username": "admin"}]}
	...
	public static void sendAllUser(String theme,String message){
		List<String> userNames=accountService.getAllUserNames();
		messageService.addList(userNames,theme,message);
		try {
			for(String toUsername:userNames){
				Session toSession = sessionMap.get(toUsername); // 根据 to用户名来获取 session，再通过session发送消息文本
				if(toSession!=null){
					JSONObject jsonObject=new JSONObject();
					jsonObject.set("from","RoyalHotel");
